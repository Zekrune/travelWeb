...
public class GPTService {
  ...
  private String generatePrompt(TravelPlan plan) {
    return """
                ...
                당신의 역할은 사용자 정보를 바탕으로 실행 가능한 고품질의 현실적인 여행 일정을 생성하는 것입니다. 여행자의 출발지와 목적지, 여행 기간, 여행테마, 예상 예산, 이동 수단 등을 고려하여, 다음 조건을 충족하는 여행 일정을 구성하세요.
                ...
                * 식사 및 휴식 시간 고려: 현지 인기 맛집과 카페 정확한 장소 추천, 무리 없는 일정 조정
                ...
                여행 계획은 반드시 다음 기준을 엄격하게 지켜 구성하세요:

                1. 부산 내 실제 장소만 사용
                - 일정에 포함되는 **모든 장소는 부산광역시 내 주소를 가진 장소여야 합니다**.
                - 주소지 기준 **우편번호가 46~49로 시작하는 장소**만 포함할 수 있습니다.
                - **다른 지역(예: 서울)에 동일한 이름이 있는 장소**는 포함할 수 없습니다.
                - 예를 들어 ‘송도해수욕장’은 부산 송도만 포함할 수 있으며, **서울 송도는 절대 포함 불가**입니다.

                2. 카카오맵에서 검색 가능한 정확한 장소명만 사용
                - 모든 장소는 반드시 **카카오맵에서 실제로 검색 가능한 명칭**이어야 합니다.
                - 추상적이고 일반적인 표현(예: ‘현지 맛집’, ‘분위기 좋은 카페’)은 절대 사용하지 마세요.
                - 프랜차이즈 카페나 음식점의 경우 반드시 **지점명을 포함**하여 표기하세요.
                  - 예: ‘스타벅스 부산 해운대점’, ‘이디야커피 광안리해수욕장점’

                3. 여행 일정 구성 기본 규칙
                - 도착일 첫날은 교통수단이 ‘기차’, ‘비행기’, ‘버스’ 인경우 기차는 서울역, 비행기는 공항, 버스는 터미널에서 시작하며 그 외의 수단일 경우 출발지 언급없이 관광지부터 시작해주세요.
                - 숙소 체크인은 14시 이후를 기준으로 하여 일정을 구성하세요.
                - 둘째 날부터 마지막 전날까지는 숙소에서 시작해 숙소로 돌아오는 일정으로 구성하고, 각 일자마다 동일한 지역 중심(예: 해운대구 중심, 중구 중심 등)으로 구성하세요.
                - 마지막 날은 숙소에서 출발하되, 마무리는 교통 수단에 따라 지정된 장소로 종료하세요:
                  - 기차: 부산역 / 버스: 부산사상버스터미널 / 비행기: 김해공항
                - 각 일정은 이동 동선을 고려하여 효율적으로 구성하며, 도보·대중교통·차량 등의 이동 시간을 반영해 주세요.

                4. 식사·카페도 반드시 같은 지역 내에서 선정
                ...
                9. 숙소 정보 포함
                - 여행자가 숙박하는 지역(예: 남포동, 해운대 등)에 맞는 **추천 호텔을 포함**하세요.
                - 카카오맵에서 검색 가능한 호텔명을 사용하세요.
                                ...
                                "schedule": [
                                  {
                                    "departureTime": "출발 시간",
                                    "category": "카테고리",
                                    "location": "장소 이동 정보 (정확한 명칭 + 지역 지점 포함)",
