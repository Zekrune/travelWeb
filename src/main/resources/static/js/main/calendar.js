...
function showYearCalendar() {
  ...
  // 연간 달력 컨테이너가 없으면 생성
  let yearCalendarContainer = document.getElementById("year-calendar-container");
  if (!yearCalendarContainer) {
    ...
    document.getElementById("calendar-container").appendChild(yearCalendarContainer);
  } else {
  ...
  // 이벤트 리스너 추가
  document.querySelector(".year-close-btn").addEventListener("click", function() {
    yearCalendarContainer.style.display = "none";
    document.getElementById("calendar").style.display = "block";
  });
  
  document.querySelector(".year-prev-btn").addEventListener("click", function() {
    updateYearCalendar(parseInt(document.querySelector(".year-title").textContent) - 1);
  });
  
  document.querySelector(".year-next-btn").addEventListener("click", function() {
    updateYearCalendar(parseInt(document.querySelector(".year-title").textContent) + 1);
  });
}
